    ...
    >
      ...
      >
        <div className="flex h-14 items-center justify-between border-b border-white/10 px-4">
          {mobileView !== "root" ? (
            <button onClick={() => setMobileView("root")} className="-m-2 p-2 text-white/85" aria-label="Back">
          ...
          <span className="text-xs tracking-[0.18em] text-white/60 uppercase">{title}</span>
          <span className="h-8 w-8" />
        </div>
        <div className="relative flex-1 overflow-hidden">
          <div className="flex h-full w-[200%] duration-300 ease-out motion-safe:transition-transform" style={{ transform: `translateX(-${viewIndex * 100}%)` }}>
            <section className="h-full min-w-full space-y-5 overflow-y-auto px-4 py-5">
              ...
              <Link href="/showroom" onClick={() => setOpenMobile(false)} className="nav-link block text-white">Showroom</Link>
            </section>
            <section className="h-full min-w-full overflow-y-auto px-4 py-5">
              ...
              <ul className="space-y-3 text-white/90">
                ...
                <li><Link href="/shop/women" onClick={() => setOpenMobile(false)} className="nav-link block">Women</Link></li>
                <li><Link href="/shop/men" onClick={() => setOpenMobile(false)} className="nav-link block">Men</Link></li>
                <li><Link href="/shop/unisex" onClick={() => setOpenMobile(false)} className="nav-link block">Unisex</Link></li>
                <li><Link href="/shop/gifts" onClick={() => setOpenMobile(false)} className="nav-link block">Gifts</Link></li>
              </ul>
            </section>
          </div>
        </div>
